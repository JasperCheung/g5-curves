...
/*======== struct matrix * make_bezier() ==========
  ...
  ====================*/
struct matrix * make_bezier() {
  struct matrix* b_easy = new_matrix(4,4);
  b_easy->lastcol = 4;
  ident(b_easy);
  b_easy->m[0][0] = -1;
  b_easy->m[0][1] = 3;
...
/*======== struct matrix * make_hermite() ==========
  ...
  ====================*/
struct matrix * make_hermite() {
  struct matrix* du_g = new_matrix(4,4);
  ident(du_g);
  du_g->lastcol = 4;
  du_g->m[0][0] = 2;
  du_g->m[0][1] = -2;
...
struct matrix * generate_curve_coefs( double p1, double p2, 
  ...
  if(type == HERMITE) {
    steadfast = make_hermite();
  }else{
    steadfast = make_bezier();
  }
  matrix_mult(steadfast, temp);
  free_matrix(steadfast);
  return temp;
}
